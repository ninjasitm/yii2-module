...
?>
		<div class="form-group">
			...
			<?=
				...
				['class' => 'form-control'])->label("Engine", ['class' => 'sr-only']); 
			?>
			<?php 
				echo Html::activeHiddenInput($model, 'convert[from]', ['value' => $model->engine]);
				echo Html::activeHiddenInput($model, 'convert[do]', ['value' => true]);
				echo Html::activeHiddenInput($model, 'engine', ['value' => $model->engine]);
...
<?php $form = ActiveForm::begin(['id' => 'container_create',
		...
		<div class="form-group">
		<?=
				$form->field($model, 'value')->textInput(['placeholder' => 'Container name...'])->label("Create a new container");
		?>
		<?php
				echo Html::activeHiddenInput($model, 'what', ['value' => 'container']);
				echo Html::submitButton('Create Container', ['class' => 'btn btn-primary pull-right']);
		?>
		</div>
<?php ActiveForm::end(); ?>
</div> 
